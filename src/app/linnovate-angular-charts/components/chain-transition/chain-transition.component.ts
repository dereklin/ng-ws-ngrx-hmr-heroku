...
export class ChainTransitionComponent implements ChartComponent, OnInit, OnChanges {
  ...
  private renderMain() {
    ...
    const line = d3.line<{asOf: any, px: any}>()
      .x((d) => this.xScale(d.asOf))
      .y((d) => this.yScaleRight(d.px));
    this.xScale.domain([this._data[0].asOf, _.last(this._data).asOf]);
    this.yScaleLeft.domain(d3.extent(this._data, (d) => d.deltaVol));
    this.yScaleRight.domain(d3.extent(this._data, (d) => d.px));
  ...
  private renderNav() {
    ...
    this.navYAxis1
      .attr('transform', 'translate(' + this.width + ' ,0)');
    const xAxis = d3.axisBottom(this.xScale);
    const yAxisLeft = d3.axisLeft(this.navYScale);
    const line = d3.line<{asOf: any, deltaVol: any}>()
      .x((d) => this.xScale(d.asOf))
      .y((d) => this.navYScale(d.deltaVol));
    this.xScale.domain([this._data[0].asOf, _.last(this._data).asOf]);
    this.navYScale.domain(d3.extent(this._data, (d) => d.deltaVol));
  ...
  private setup(): void {
    this.parentElement = this.parentElement || this.el.nativeElement.parentElement;
